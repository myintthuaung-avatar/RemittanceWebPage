...
export default {
    ...
    methods: {
        ...
        EditItem(item) {
            var currency = this;
            currency.ShowLoading(true);
            Service.GetById(item.id).then(
                (response) => {
                    window.console.log(response.data)
                    currency.$refs.EntryModal.model = response.data;
                    currency.$refs.EntryModal.disabled = true;

                    currency.ShowLoading(false);
                    currency.$refs.EntryModal.Show("edit");
                });
        },

        DeleteItem(item) {
            var currency = this;
            currency.$root.ConfirmBoxModal.open().then(res => {
                if (res) {
                    currency.ShowLoading(true);
