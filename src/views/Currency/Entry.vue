...
<script>
import Model from "@/models/currency.js";
...
export default {
    data() {
        return {
            ...
            dialog: false,
            model: new Model(),
            coderule: [(v) => !!v || "Please Fill Currency Code !"],
            namerule: [(v) => !!v || "Please Fill Currency Name !"],
        }
    },
    ...
    methods: {
        ...
        Show(fname) {
            this.dialog = true;
            if (fname == "new") {
                this.formname = "Currency Entry";
            } else {
                this.formname = "Edit Currency Entry";
            }
        },
        ValidateModel() {
        ...
        SaveOrUpdate() {
            var currency = this;
            currency.ValidateModel();
            if (currency.valid == true) {
                Service.SaveOrUpdate(currency.model).then((result) => {
                    if (result.data) {
                        var message = result.data.messages[0];
                        if (result.data.success) {
                            currency.$root.AlertDialog.Show({ message: message, color: 'success' });
                            currency.$emit("AfterSaved");
                            currency.ClearData();
                        } else {
                            currency.$root.AlertDialog.Show({ message: message, color: 'error' });
                        }
                    }
                });
            }
