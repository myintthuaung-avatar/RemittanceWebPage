...
import Service from "@/services/cashBookType.service";
...
export default {
    ...
    methods: {
        ...
        EditItem(item) {
            var cbt = this;
            cbt.ShowLoading(true);
            Service.GetById(item.id).then(
                (response) => {
                    window.console.log(response.data)
                    cbt.$refs.EntryModal.model = response.data;
                    cbt.$refs.EntryModal.disabled = true;

                    cbt.ShowLoading(false);
                    cbt.$refs.EntryModal.Show("edit");
                });
        },
                        ...
                        (response) => {
                            if (response.data) {
                                ...
                                if (response.data.success) {
                                    cbt.$root.AlertDialog.Show({ message: message, color: 'success' });
                                    cbt.ResetFilter();

                                } else {
                                    cbt.$root.AlertDialog.Show({ message: message, color: 'error' });
                                }
                            }
                        },
                        (error) => {
