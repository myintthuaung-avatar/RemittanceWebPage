    ...
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'StateDivision', key: 'state_division_name', width: 220 },
        ...
        EditItem(item) {
            ...
            Service.GetById(item.id).then(
                (response) => {
                    ...
                    ct.$refs.EntryModal.model = response.data;
                    ct.$refs.EntryModal.disabled = true;

                    ct.ShowLoading(false);
                    ct.$refs.EntryModal.Show("edit");
                });
        },

        DeleteItem(item) {
            var ct = this;
            ct.$root.ConfirmBoxModal.open().then(res => {
                if (res) {
                    ct.ShowLoading(true);
                        ...
                        (response) => {
                            if (response.data) {
                                ...
                                if (response.data.success) {
                                    ct.$root.AlertDialog.Show({ message: message, color: 'success' });
                                    ct.ResetFilter();

                                } else {
                                    ct.$root.AlertDialog.Show({ message: message, color: 'error' });
                                }
                            }
                        },
                        (error) => {
